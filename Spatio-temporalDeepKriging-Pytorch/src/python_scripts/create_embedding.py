...
def main():
    ...
    for j in range(N_data):
        ...
        for res in range(len(num_basis_time)):
            ...
            for i in range(num_basis_time[res]):
                d = np.square(np.absolute(s_j - mu_knots_time[res][i]))
                if d >= 0 and d <= 1:
                    phi_t_row[0,i+K_time]=(np.exp(-0.5 * d / (std**2)))
                else:
                    phi_t_row[0,i+K_time]=0
        ...
        for res in range(len(num_basis_space)):
            ...
            for i in range(num_basis_space[res]):
                d = np.linalg.norm(s_j_2d - knots[i, :], axis=1) / theta
                if d >= 0 and d <= 1:
